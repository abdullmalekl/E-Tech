...
const Part2 = () => {
  ...
  useEffect(() => {
    // قم بنقل العنصر المستهدف بعد تحميل الصفحة
  ...
  const toPart2 = async(e)=>{
    ...
    try {
        const res = await axios.post('https://etech.justhost.ly/api/lessons/parts', formData);
        setWaiting(false);
        if (res.status === 201) {
          localStorage.removeItem("savedHtml");
          Navigate('/Dashboard/lessons/3');
        }
      } catch (error) {
        setWaiting(false);
        Swal.fire({
