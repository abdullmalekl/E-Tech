...
const Fprojects = () => {
  ...
  const toPart2 = async(e)=>{
    ...
    try {
      ...
      if(!localStorage.getItem("title")){
        res = await multiPart.post('PlatformProjects', formData);
      }else{
        res = await multiPart.put(`PlatformProjects/${localStorage.getItem("id")}`, formData);
      }
      setWaiting(false);
      if (res.status === 201) {
        localStorage.removeItem("savedHtml");
        Navigate('/Dashboard/projects/2');
      }
    } catch (error) {
      setWaiting(false);
      Swal.fire({
  ...
  const sendData = async(e)=>{
    ...
    try {
      var res = null;
      if(!localStorage.getItem("title")){
        res = await axios.post('https://etech.justhost.ly/api/PlatformProjects', formData);
      }else{
         await multiPart.put(`PlatformProjects/${localStorage.getItem("id")}`, formData);
      }
      setWaiting(false);
      if (res.status === 201) {
        localStorage.removeItem("savedHtml");
  ...
  const url = `https://etech.justhost.ly/storage/app/public/projects/images/`;
 ...
 <Fragment>
  ...
  <div style={{ width:1192.5, height: 2000 , backgroundColor: '#F3F3F3'}}>
      ...
      {localStorage.getItem("title")? 'تعديل مشروع  / الجزء الأول' : 'مشروع جديد / الجزء الأول'}
      </div>
...
<FroalaEditor  
     ...
     {!waiting?  <button onClick={sendData} id='successButton' className='btn btn-success'>
      ...
      {!waiting? <button onClick={cancelHandler} id='dangerButton' className='btn btn-danger'>
         الغاء 
      </button> : <button disabled onClick={()=> Navigate('/Dashboard/projects')} id='dangerButton' className='btn btn-danger'>
        انتظر...
      </button>}
